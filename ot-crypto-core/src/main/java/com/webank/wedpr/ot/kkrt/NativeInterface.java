...
class NativeInterface {
    ...
    //         // Windows based
    //         System.out.println("windows ot is not support! ");
    //         // return 0;
    ...
    //                             .getPath("./libmiracl.so") // Dynamic link
    //                             .normalize()
    //                             .toAbsolutePath()
    //                             .toString());
    //             System.load(
    ...
    //             System.load(
    //                     FileSystems.getDefault()
    //                             .getPath("./libcryptoTools.so") // Dynamic link
    //                             .normalize()
    //                             .toAbsolutePath()
    //                             .toString());
    //             System.load(
    //                     FileSystems.getDefault()
    //                             .getPath("./liblibOTe.so") // Dynamic link
    //                             .normalize()
    //                             .toAbsolutePath()
    //                             .toString());
    ...
    // handle
    private static native long newSender(
            long choiceCount, long messageCount, String[] data, long[] key);

    private static native void deleteSender(long senderHandle);

    private static native void getSenderEncryptedMessage(
            long senderHandle, long[][] enMessage, byte[][] hash);
    ...
    private static native void step1ReceiverInitBaseOt(
            long receiverHandle, byte[] senderPackSeed, long[] receiverSeed);

    private static native void step2SenderExtendSeedPack(
            long senderHandle, byte[] senderPackSeed, long[] receiverSeed, byte[] receiverPack);

    private static native void step3ReceiverSetSeedPack(long receiverHandle, byte[] receiverPack);

    private static native void step4SenderGenerateSeed(
            long senderHandle, long[] senderSeed, byte[] senderSeedHash);

    private static native void step5ReceiverInitMatrix(
            long receiverHandle,
            long[] senderSeed,
            byte[] senderSeedHash,
            long[] receiverSeed,
    ...
    private static native void step6SenderSetMatrix(
            long senderHandle,
            long[] receiverSeed,
            long[] receiverMatrix,
            long[] senderSeed,
            long[] senderMatrix);
    // so many step7
    private static native void step7ReceiverGetFinalResultWithDecMessage(
            long receiverHandle,
            long[] senderMatrix,
            long[][] enMessage,
            byte[][] hash,
    ...
    public static OtData step2SenderExtendSeedPack(long senderHandle, OtData otData) {
        ...
        step2SenderExtendSeedPack(
                senderHandle, otData.senderPackSeed, otData.receiverSeed, otData.receiverPack);
        step4SenderGenerateSeed(senderHandle, otData.senderSeed, otData.senderSeedHash);
        return otData;
    }

    public static OtData step3ReceiverInitMatrix(
            long receiverHandle, OtData otData, int choiceCount) {
        otData.receiverSeed = new long[BLOCK_LONG_SIZE];
        ...
        step5ReceiverInitMatrix(
                receiverHandle,
                otData.senderSeed,
                otData.senderSeedHash,
                otData.receiverSeed,
                otData.receiverMatrix);
        return otData;
    }

    public static OtData step4SenderSetMatrix(
            long senderHandle, OtData otData, int choiceCount, int messageCount) {
        int senderMatrixSize = BLOCK_LONG_SIZE * choiceCount * messageCount;
        otData.senderMatrix = new long[senderMatrixSize];
        step6SenderSetMatrix(
                senderHandle,
                otData.receiverSeed,
                otData.receiverMatrix,
                otData.senderSeed,
    ...
    public static OtResult step5ReceiverGetFinalResultWithDecMessage(
            long receiverHandle, OtData otData, int choiceCount) {
        OtResult otResult = new OtResult();
        otResult.data = new String[choiceCount];
        step7ReceiverGetFinalResultWithDecMessage(
                receiverHandle, otData.senderMatrix, otData.enMessage, otData.hash, otResult.data);
        return otResult;
    }
    ...
    public static void main(String[] args) {
        ...
        long[] receiverMatrix =
                new long[RECEIVER_MATRIX_COLUMN_NUMBER * BLOCK_LONG_SIZE * choiceCount];
        step5ReceiverInitMatrix(
                receiverHandle, senderSeed, senderSeedHash, receiverSeedKkrt, receiverMatrix);
        ...
        step6SenderSetMatrix(
                senderHandle, receiverSeedKkrt, receiverMatrix, senderSeed, senderMatrix);
        ...
        step7ReceiverGetFinalResultWithDecMessage(
                receiverHandle, senderMatrix, enMessage, hash, data);
